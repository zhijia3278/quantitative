...
class LexinSport:
    ...
    def login(self):
        ...
        # 如果在请求时显示请求超出最大次数，需要在请求中设置verify=False
...
def get_sleep_time():
    ...
    # 第二天6点时间戳
    tomorrow_run_time = int(time.mktime(time.strptime(str(tomorrow), '%Y-%m-%d'))) + 21600
...
if __name__ == "__main__":
    step = random.randint(28000, 50000)
    # 最大运行出错次数
    fail_num = 1
    while 1:
        while fail_num > 0:
            try:
                # 修改步数结果
                result = LexinSport('15063777008', '555555', step).change_step()
                # print(result)
                break
        ...
        print ("任务完成，当前作业时间是====>" + str(datetime.datetime.now()) + ",计算出来的睡眠时间是====>" + str(sleep_time))
